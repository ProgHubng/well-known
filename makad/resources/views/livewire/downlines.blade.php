...
<h1 class="text-xl font-medium text-blue-700">My Level 1 Team Members</h1>
</section>
        ...
        <table class="w-full table-auto bg-cyan-100 rounded-xl">
            <thead>
                <tr class="text-sm text-left text-blue-700">
                    ...
                    <th class="px-4 py-3">Downlines</th>
                    <th class="px-4 py-3">Deposits</th>
                    <th class="px-4 py-3">Withdrawals</th>
            ...
            <tbody class="text-blue-700 text-sm">
                @foreach ($downline as $item)
                    ...
                    $deposits = App\Model\Purchased_plan::where('user_id', $item->user->id)->where('status', 'success')->sum('price');
                    <td>{{ $deposits }}</td>

                    $withdrawals = App\Model\Withdrawal::where('user_id', $item->user->id)->where('status', 'success')->sum('amount');
                    <td>{{ $withdrawals }}</td>
                @endforeach
            </tbody>
        </table>
